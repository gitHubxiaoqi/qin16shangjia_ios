...
@interface MainViewController ()<UIAlertViewDelegate,UITableViewDelegate,UITableViewDataSource, UINavigationControllerDelegate, UIImagePickerControllerDelegate, SRRefreshDelegate, IChatManagerDelegate, DXChatBarMoreViewDelegate, DXMessageToolBarDelegate,IDeviceManagerDelegate>
...
- (void)viewDidLoad {
    ...
    deleteBtn.frame=CGRectMake(self.view.bounds.size.width-50, 2, 50, 40);
    [deleteBtn addTarget:self action:@selector(removeAllMessages:) forControlEvents:UIControlEventTouchUpInside];
    [navBgImgview addSubview:deleteBtn];
    
    UIImageView * jiantouImgView=[[UIImageView alloc] initWithImage:[UIImage imageNamed:@"msg_delete_post.png"]];
    jiantouImgView.frame=CGRectMake(20, 12,20, 20);
    [deleteBtn addSubview:jiantouImgView];
...
- (void)removeAllMessages:(id)sender
{
    if (_dataSource.count == 0) {
        [self showHint:NSLocalizedString(@"没有消息", @"no messages")];
        return;
    }
    ...
    if ([sender isKindOfClass:[NSNotification class]]) {
        ...
        [WCAlertView showAlertWithTitle:NSLocalizedString(@"提示", @"Prompt")
                                message:NSLocalizedString(@"确定要删除与对方的所有聊天记录？", @"please make sure to delete")
         ...
         } cancelButtonTitle:NSLocalizedString(@"取消", @"Cancel") otherButtonTitles:NSLocalizedString(@"确定", @"OK"), nil];
    }
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (indexPath.row < [self.dataSource count]) {
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            NSDictionary * dic=[[NSDictionary alloc] initWithDictionary:model.message.ext];
            NSString * picStr=[dic objectForKey:@"senderPic"];
            //cell.messageModel.headImageURL=[NSURL URLWithString:picStr];
            UIImage *placeholderImage = [UIImage imageNamed:@"chatListCellHead"];
            [cell.headImageView setImageWithURL:[NSURL URLWithString:picStr] placeholderImage:placeholderImage];
    ...
    if ([model.messageBody messageBodyType] == eMessageBodyType_Image) {
        ...
        if (imageBody.thumbnailDownloadStatus == EMAttachmentDownloadSuccessed) {
            ...
            [chatManager asyncFetchMessage:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                ...
                [weakSelf showHint:NSLocalizedString(@"获取图像失败", @"image for failure!")];
            } onQueue:nil];
        }else{
            //获取缩略图
            [chatManager asyncFetchMessageThumbnail:model.message progress:nil completion:^(EMMessage *aMessage, EMError *error) {
                if (!error) {
                    ...
                    [weakSelf showHint:NSLocalizedString(@"获取图像失败", @"thumbnail for failure!")];
                }
...
-(void)chatAudioCellBubblePressed:(MessageModel *)model
{
    ...
    if (downloadStatus == EMAttachmentDownloading) {
        [self showHint:NSLocalizedString(@"点击下载语音", @"downloading voice, click later")];
        return;
    }
...
{
    if ([_conversation.chatter isEqualToString:message.conversationChatter]) {
        [self showHint:NSLocalizedString(@"接收命令消息", @"receive cmd message")];
    }
}
...
- (void)moreViewTakePicAction:(DXChatBarMoreView *)moreView
{
    ...
    [self showHint:NSLocalizedString(@"模拟器不支持相机", @"simulator does not support taking picture")];
...
- (void)didStartRecordingVoiceAction:(UIView *)recordView
{
    if ([self canRecord]) {
        ...
        if (error) {
            NSLog(NSLocalizedString(@"录音失败", @"failure to start recording"));
        }
    }
}
...
- (void)downloadMessageAttachments:(MessageModel *)model
{
    void (^completion)(EMMessage *aMessage, EMError *error) = ^(EMMessage *aMessage, EMError *error) {
        ...
        {
            [self showHint:NSLocalizedString(@"获取消息失败", @"thumbnail for failure!")];
        }
    };
...
- (void)showMenuViewController:(UIView *)showInView andIndexPath:(NSIndexPath *)indexPath messageType:(MessageBodyType)messageType
{
    ...
    if (_copyMenuItem == nil) {
        _copyMenuItem = [[UIMenuItem alloc] initWithTitle:NSLocalizedString(@"拷贝", @"Copy") action:@selector(copyMenuAction:)];
    }
...
-(void)sendTextMessage:(NSString *)textMessage
{
    ...
    NSDictionary * dic=[[NSDictionary alloc] initWithObjectsAndKeys:appdelegate.appUser.headPortraitImgUrlStr,@"senderPic",appdelegate.appUser.userName,@"senderName", self.peoplePic,@"reciverPic",self.peopleName,@"reciverName",nil];
    
    EMMessage *tempMessage = [ChatSendHelper sendTextMessageWithString:textMessage
                                                            toUsername:_conversation.chatter
                                                           isChatGroup:_isChatGroup
                                                     requireEncryption:NO
                                                                   ext:[[NSDictionary alloc] initWithDictionary:dic]];
    
    //message.ext=[[NSDictionary alloc] initWithDictionary:dic];
...
-(void)sendAudioMessage:(EMChatVoice *)voice
{
    ...
    EMMessage *tempMessage = [ChatSendHelper sendVoice:voice
                                            toUsername:_conversation.chatter
                                           isChatGroup:_isChatGroup
                                     requireEncryption:NO ext:[[NSDictionary alloc] initWithDictionary:dic]];
    [self addMessage:tempMessage];
}
